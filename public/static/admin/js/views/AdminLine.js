...
function AdminLine() {
  ...
  const $subwayLineCreateButton = document.querySelector(
    "#subway-line-create-btn"
  );
  ...
  const updateSubwayLine = () => {
    ...
    api.line
      ...
      .then(() => {
        subwayLineModal.toggle();
        $activeSubwayLineItem.querySelector(".line-name").innerText =
          updatedSubwayLine.name;
      })
  ...
  const initCreateSubwayLineForm = () => {
    ...
    $colorSelectContainer.innerHTML = subwayLineColorOptions
  ...
  const initEventListeners = () => {
    $subwayLineList.addEventListener(
      EVENT_TYPE.CLICK,
      onDeleteSubwayLineHandler
    );
    $subwayLineList.addEventListener(
      EVENT_TYPE.CLICK,
      onShowUpdateSubwayLineModal
    );
    $subwayLineFormSubmitButton.addEventListener(
      EVENT_TYPE.CLICK,
      onSubmitHandler
    );
    $subwayLineCreateButton.addEventListener(
      EVENT_TYPE.CLICK,
      onCreateLineFormInitializeHandler
    );
  };
